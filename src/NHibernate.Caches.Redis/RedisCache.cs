...
");
		...
		internal readonly bool DisableIndexSetOnKeys;
		...
		public RedisCache(RedisCacheConfiguration configuration, ConnectionMultiplexer connectionMultiplexer, RedisCacheProviderOptions options)
		{
			...
			DisableIndexSetOnKeys = configuration.DisableIndexSetOnKeys ?? options.DisableIndexSetOnKeys;
			...
			log.DebugFormat("creating cache: regionName='{0}', expiration='{1}', lockTimeout='{2}', acquireLockTimeout='{3}', disableIndexSetOnKeys='{4}'",
				RegionName, expiration, lockTimeout, acquireLockTimeout, DisableIndexSetOnKeys
			);
		...
		public virtual void Put(object key, object value)
		{
			...
			try
			{
				...
				var db = GetDatabase();
				if (DisableIndexSetOnKeys)
				{
					db.StringSet(cacheKey, serializedValue, expiration, When.Always, fireAndForgetFlags);
				}
				else
				{
		...
		public virtual void Clear()
		{
			...
			try
			{
				...
				if (DisableIndexSetOnKeys)
				{
					//prefer slave for KEYS/SCAN
					var scanServer = connectionMultiplexer.GetEndPoints().Select(x => connectionMultiplexer.GetServer(x)).Where(x => x.IsConnected).OrderByDescending(x => x.IsSlave).First();
					//ToArray forces enumeration (via SCAN if supported) - it is as atomic as we get
					db.KeyDelete(scanServer.Keys(options.Database, CacheNamespace.GetKeyPattern()).ToArray());
				}
				else
				{
					var setOfActiveKeysKey = CacheNamespace.GetSetOfActiveKeysKey();
